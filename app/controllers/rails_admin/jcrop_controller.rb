  ...
  class JcropController < RailsAdmin::ApplicationController
    ...
    def edit
      ...
      if @object.send(@field).class.to_s =~ /Uploader/ and defined?(Carrierwave)
        ...
        @image_tag_options[:'data-geometry'] = geometry(@file_path).join(",") if File.exists?(@file_path)
      ...
      #Condition for Shrine.
        ...
        if defined?(Shrine::Storage::S3) and (@object.send(@field)[:original].storage.is_a?(Shrine::Storage::S3))
          ...
          @file_path=_field.url
        else

          @file_path=_field.to_io
        end

        metadata = _field.metadata rescue {}
        @image_tag_options[:'data-geometry'] = [metadata["width"], metadata["height"]].join(",")
      end

      if File.exists?(@file_path)

        if @fit_image_geometry and false
      ...
      elsif @object.try("original_#{@field}_data") and !@object.send("original_#{@field}_data").image.blank? and false
    ...
    def update
      ...
      # original file in object`s original_image as file
      elsif @object.try("original_#{params[:crop_field]}_data") and !@object.send("original_#{params[:crop_field]}_data").image.blank? and false
        _dir = File.dirname(path)
        FileUtils.mkdir_p(_dir) unless File.exists?(_dir)
        File.unlink(path) if File.symlink?(@object.send(params[:crop_field]).path)
        File.symlink(@object.send("original_#{params[:crop_field]}_data").image.path, path)
        ...
        @object.rails_admin_crop! params.merge(crop_process_before: '+repage', crop_process_after: '+repage'), upload_plugin_prefix
        File.unlink(@object.send(params[:crop_field]).path) if File.exists?(path)

      elsif @object.try("original_#{params[:crop_field]}") and false
    ...
    def upload_plugin_prefix(field = @field)
      ...
      #Condition for Paperclip.
        ...
        #Condition for Shrine.
      elsif (_field.is_a?(Hash) or _field.class.to_s =~ /UploadedFile/) and defined?(Shrine)   
